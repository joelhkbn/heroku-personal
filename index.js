...
app.use('/uploads', express.static(__dirname + '/uploads')) // untuk mengimport data-data yang diperlukan hbs
...
app.get('/blog', function (req, res) {
  let query = `SELECT tb_blog.id, tb_blog.title, tb_blog.content, tb_blog.image, tb_user.name AS author, tb_blog.post_at  FROM tb_blog  LEFT JOIN tb_user
    ...
    client.query(query, function (err, result) {
      ...
      data = data.map(function (item) {
        return {
          ...
          time_ago: getTimeAgo(item.post_at),
        }
      })
  ...
  db.connect(function (err, client, done) {
    ...
    client.query(query, function (err, result) {
      ...
      if (isMatch) {
        ...
        req.flash('danger', 'Email dan password tidak cocok! astagaaa')
        res.redirect('/login')
      }
    })
  })
